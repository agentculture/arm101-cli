...
arm101-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. arm101-cli
...
#   - consumer-identifying prose: `guildmaster` → `arm101-cli` (NOT
...
If a re-sync would lose a arm101-cli adaptation, lift the change
...
#     → `which arm101-cli vendors from guildmaster`
